...
import styles from './ComicPageScreen.module.css';
...
import {
  getComicPageContent,
  resetGetComicPageContent
} from '../../../slices/pageSlices/comicPageContentSlices/comicPageContentGetSlice';
import {
  resetUpdateComicPageContent,
  updateComicPageContent
} from '../../../slices/pageSlices/comicPageContentSlices/comicPageContentUpdateSlice';
...
export default function ComicPageScreen() {
  ...
  const { status, content, error } = comicPageContentGetSlice;

  const comicPageContentUpdateSlice = useSelector(
    (state) => state.comicPageContentUpdateSlice
  );
  const {
    status: statusUpdate,
    content: contentUpdate,
    error: errorUpdate
  } = comicPageContentUpdateSlice;

  const dispatch = useDispatch();

  useEffect(() => {
    return () => {
      dispatch(resetUpdateComicPageContent());
    };
  }, [dispatch]);

  useEffect(() => {
    return () => {
      dispatch(resetGetComicPageContent());
    };
  }, [dispatch]);
  ...
  return (
    <div className={styles.main_wrapper}>
      ...
      <div className={styles.hero_section}>
        <h1 className={styles.heading}>Edit Comic Page</h1>
      </div>
